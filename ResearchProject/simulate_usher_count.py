...
avg_wait_time = run_simulation(NUM_USHERS)
while avg_wait_time > 10 and NUM_USHERS < 20:
    avg_wait_time = run_simulation(NUM_USHERS)
    wait_times_list.append(round(avg_wait_time, 2))
    print("Average wait time with", NUM_USHERS, "cashiers:", round(avg_wait_time, 2), "minutes")
    NUM_USHERS += 1
plt.plot(range(2, NUM_USHERS), wait_times_list, marker='o', linestyle='-', color='b')
